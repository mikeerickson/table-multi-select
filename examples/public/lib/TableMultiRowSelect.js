...
( function( window, undefined ) {
  ...
  function TableMultiRowSelect(options) {
    ...
    _bootstrap(this, defaultOptions);
  ...
  function _bootstrap(instance, options) {

    var that = instance;

    var tableObj = $('#' + options.id );
    if ( tableObj.length === 0 ) {
      tableObj   = $('.' + options.className);
    }
    ...
    if(tableRows) {
      ...
      console.error('Unable to find table reference. Make sure you have a table with id or class `%s`.', options.className);
    }
